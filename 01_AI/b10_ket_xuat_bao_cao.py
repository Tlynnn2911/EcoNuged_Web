...
================================================
Đã fix font chữ Unicode cho PDF (hỗ trợ tiếng Việt)
"""
...
class KetXuatBaoCaoESG:
    ...
    def xuat_pdf(self) -> str:
        try:
            ...
            if not os.path.exists(font_path):
                # Nếu không có file font, thử dùng font mặc định (có thể lỗi)
                print("[B10] ⚠️ Không tìm thấy DejaVuSans.ttf, font có thể không hiển thị tiếng Việt")
            ...
            style_normal = ParagraphStyle('Normal', parent=styles['Normal'], fontName='DejaVuSans', fontSize=10, leading=14)
            style_title = ParagraphStyle('Title', parent=styles['Title'], fontName='DejaVuSans', fontSize=18, textColor=colors.HexColor('#1A7A4A'), alignment=TA_CENTER)
            style_heading = ParagraphStyle('Heading2', parent=styles['Heading2'], fontName='DejaVuSans', fontSize=13, textColor=colors.HexColor('#1A7A4A'))
            style_footer = ParagraphStyle('Footer', parent=styles['Normal'], fontName='DejaVuSans', fontSize=8, textColor=colors.grey, alignment=TA_CENTER)
        ...
        story = []
        story.append(Paragraph("BÁO CÁO PHÁT THẢI KHÍ NHÀ KÍNH", style_title))
        story.append(Paragraph("EcoNudge Gate – Hành lang Logistics Hải Phòng", style_footer))
        story.append(Paragraph(f"Kỳ báo cáo: {tom_tat['ky_bao_cao']}  |  Xuất ngày: {datetime.now().strftime('%d/%m/%Y %H:%M')}", style_footer))
        story.append(HRFlowable(width="100%", thickness=1.5, color=colors.HexColor('#1A7A4A')))
        story.append(Spacer(1, 0.4*cm))

        story.append(Paragraph("1. PHẠM VI & TIÊU CHUẨN ÁP DỤNG", style_heading))
        ...
        story.append(Paragraph("2. KẾT QUẢ PHÁT THẢI TỔNG HỢP", style_heading))
        data_tt = [
            ["Chỉ số", "Giá trị"],
        ...
        story.append(Paragraph("3. PHÂN BỔ PHÁT THẢI THEO CỤM CẢNG", style_heading))
    ...
    def xuat_toan_bo(self) -> dict:
        ...
        return {"excel": self.xuat_excel(), "pdf": self.xuat_pdf()}